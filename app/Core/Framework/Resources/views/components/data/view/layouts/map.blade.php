...
@endphp
@if($items->isEmpty())
    <x-core::data.view.partials.empty icon="map"/>
@else
    ...
    <div 
        ...
        <div class="absolute top-6 left-6 right-6 z-1000 flex justify-between items-center pointer-events-none">
            <div class="flex items-center gap-3 bg-white/90 backdrop-blur-md p-2 pl-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 dark:bg-zinc-900 shadow-xl pointer-events-auto">
                <div class="flex flex-col">
                    <span class="text-[10px] font-bold text-zinc-400 uppercase tracking-tighter">Flux Logistique</span>
                    <span class="text-xs font-bold text-zinc-900 dark:text-white">{{ $items->total() }} points actifs</span>
                </div>
