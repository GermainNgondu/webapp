        ...
        <flux:card wire:key="grid-item-{{ $item->id }}" class="p-0 overflow-hidden flex flex-col h-full hover:border-zinc-500 transition-colors">
            ...
            @if(isset($grid['image']))
                <div class="relative aspect-video w-full overflow-hidden bg-zinc-800 border-b border-zinc-800">
                    ...
                    {{-- Optionnel : On peut superposer le Badge sur l'image --}}
                    @if(isset($grid['badge']))
                        <div class="absolute top-3 right-3">
                            <x-dynamic-component :component="'project-status-badge'" :value="$item->{$grid['badge']['field']}" />
                        </div>
                    @endif
                </div>
            @endif
            ...
            <div class="p-4 flex flex-col flex-1">
                <div class="mb-2 flex items-center justify-between">
                    ...
                    <x-dataview.actions.row :actions="$actions['row'] ?? []" :item="$item" :grid="true" />
                </div>
